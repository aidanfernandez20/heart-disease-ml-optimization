...
# -------------------------------------------------------------------
# ROC CURVE: Optimized KNN ONLY
# -------------------------------------------------------------------

# Make sure best_knn is already defined and tuned (Step 4A)
best_knn.fit(X_train_scaled, y_train)

# Predicted probabilities for class 1 (HeartDisease = 1)
...
plt.plot(fpr_knn, tpr_knn, label=f"Optimized KNN (AUC = {auc_knn:.2f})")
...
# -------------------------------------------------------------------
# ROC CURVE: Optimized Random Forest ONLY
# -------------------------------------------------------------------

# Make sure best_rf is already defined and tuned (Step 4A)
...
# Fit grids and get best estimators
grid_knn.fit(X_train_scaled, y_train)
best_knn = grid_knn.best_estimator_

print("\n--- Hyperparameter Tuning for KNN (optimize Recall) ---")
print("Best KNN params:", grid_knn.best_params_)
print(f"Best KNN CV Recall: {grid_knn.best_score_:.4f}")
...
# -------------------------------------------------------------------
# STEP 7: CONFUSION MATRICES AT F2-OPTIMIZED THRESHOLDS
# -------------------------------------------------------------------
...
# --- KNN at threshold 0.23 ---
y_proba_knn = best_knn.predict_proba(X_test_scaled)[:, 1]
y_pred_knn_f2 = (y_proba_knn >= thr_knn_f2).astype(int)

cm_knn_f2 = confusion_matrix(y_test, y_pred_knn_f2)
tn_k, fp_k, fn_k, tp_k = cm_knn_f2.ravel()

print("\n=== Optimized KNN @ threshold 0.23 (F2-optimized) ===")
print("Confusion matrix (rows = actual, cols = predicted):")
print(cm_knn_f2)
print(f"TP = {tp_k}, FN = {fn_k}, TN = {tn_k}, FP = {fp_k}")
print(f"Precision = {precision_score(y_test, y_pred_knn_f2):.3f}")
print(f"Recall    = {recall_score(y_test, y_pred_knn_f2):.3f}")
print(f"F2-score  = {fbeta_score(y_test, y_pred_knn_f2, beta=2):.3f}")

# --- Random Forest at threshold 0.25 ---
y_proba_rf = best_rf.predict_proba(X_test_scaled)[:, 1]
y_pred_rf_f2 = (y_proba_rf >= thr_rf_f2).astype(int)

cm_rf_f2 = confusion_matrix(y_test, y_pred_rf_f2)
tn_r, fp_r, fn_r, tp_r = cm_rf_f2.ravel()

print("\n=== Optimized Random Forest @ threshold 0.25 (F2-optimized) ===")
print("Confusion matrix (rows = actual, cols = predicted):")
print(cm_rf_f2)
print(f"TP = {tp_r}, FN = {fn_r}, TN = {tn_r}, FP = {fp_r}")
print(f"Precision = {precision_score(y_test, y_pred_rf_f2):.3f}")
print(f"Recall    = {recall_score(y_test, y_pred_rf_f2):.3f}")
print(f"F2-score  = {fbeta_score(y_test, y_pred_rf_f2, beta=2):.3f}")

# KNN confusion matrix heatmap
plt.figure(figsize=(4, 3))
sns.heatmap(cm_knn_f2, annot=True, fmt="d", cmap="Blues")
plt.xlabel("Predicted")
plt.ylabel("Actual")
plt.title("Optimized KNN @ threshold 0.23")
plt.tight_layout()
plt.show()

# Random Forest confusion matrix heatmap
plt.figure(figsize=(4, 3))
sns.heatmap(cm_rf_f2, annot=True, fmt="d", cmap="Blues")
plt.xlabel("Predicted")
plt.ylabel("Actual")
plt.title("Optimized Random Forest @ threshold 0.25")
plt.tight_layout()
plt.show()